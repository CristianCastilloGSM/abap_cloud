CLASS zcl_estructuras_018 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_estructuras_018 IMPLEMENTATION.
...
**********************************************************************
    ...
    TYPES: BEGIN OF stest,
             airport_from_id TYPE /dmo/airport_from_id,
             airport_to_id   TYPE /dmo/airport_to_id,
             carrier_name    TYPE /dmo/carrier_name,
           END OF stest.

    DATA stest TYPE st_connection.
    DATA STEST1 TYPE TABLE OF /DMO/I_Connection.
...
*    FIELDS DepartureAirport, DestinationAirport, \_Airline-Name
    ...
    out->write(  `---------------------------------------` ).
    out->write(  `Example 5: test` ).
*
    loop at stest1 into data(result).
    out->write( result ).
    endloop.
